...
const WARRAY : [&str; 451]= [
        ...
        "want", "because", "any", "these", "give", "day", "most", "us", "where", "down",
        "out", "up", "same", "such", "here", "find", "place", "why", "those", "her",
        "little", "own", "other", "such", "still", "through", "between", "life", "being", "few",
        "might", "again", "few", "leave", "might", "right", "always", "part", "great", "last",
        "number", "own", "both", "high", "every", "man", "every", "under", "most", "after",
        "things", "might", "thing", "might", "away", "next", "work", "night", "those", "off",
        "under", "long", "see", "own", "find", "right", "end", "work", "long", "even", "own",
        "only", "through", "work", "again", "under", "both", "start", "good", "both", "two",
...
pub fn charray(len : u8, capl : bool, n : bool, s : bool) -> String {
    ...
    if capl {chances.push(Arr::Larray)}
    if n {chances.push(Arr::Narray)}
    if s {chances.push(Arr::Sarray)}
    ...
    for _i in 0..len {
        ...
        let char = match chartype {
            Arr::Larray => {
                match LARRAY.choose(&mut rand::thread_rng()) {
                    Some(x) => {
                        if capl {chances.push(Arr::CapLarray)}
                        if n {chances.push(Arr::Narray)}
                        if s {chances.push(Arr::Sarray)}
                        *x
                    },
            ...
            Arr::CapLarray => {
                match CAPLARRAY.choose(&mut rand::thread_rng()) {
                    Some(x) => {
            ...
            Arr::Narray => {
                match NARRAY.choose(&mut rand::thread_rng()) {
                    Some(x) => {
                        chances.push(Arr::Larray);
                        if capl {chances.push(Arr::CapLarray)}
                        if s {chances.push(Arr::Sarray)}
            ...
            Arr::Sarray => {
                match SARRAY.choose(&mut rand::thread_rng()) {
                    Some(x) => {
                        chances.push(Arr::Larray);
                        if capl {chances.push(Arr::CapLarray)}
                        if n {chances.push(Arr::Narray)}
...
pub fn warray (len : u8, capital : bool, numbers : bool, symbols : bool, delimeter : char) -> String {
    ...
    let mut i : u8 =  0;
    let mut chances = vec![];
    if capital {chances.push(Arr::Larray)};
    if numbers {chances.push(Arr::Narray)};
    if symbols {chances.push(Arr::Sarray)};

    while  i < len {
        let word = WARRAY.choose(&mut rand::thread_rng()).unwrap();
        if i+1 < len && i != 0 {oreo.push(delimeter); i+=1 ;} else if i==0 {} else {break}
